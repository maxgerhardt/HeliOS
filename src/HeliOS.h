 ...
 */
   ...
   *                   (volatile xAddr *). On failure, this pointer is not
   *                   modified.
   ...
   * @endcode
   *
   * @param[out] queue_ Pointer to xQueue variable that will receive the queue
   ...
   * @code xQueue queues[MAX_CHANNELS];
   * xBase queueCount = 0;
   *
   ...
   *                      created with xQueueCreate().
   * @param[out] message_ Pointer to xQueueMessage structure receiving a copy of
   ...
   * 1. Check if queue has messages (not empty) 2. Allocate xQueueMessage
   ...
   * commandProcessorTask(xTask task, xTaskParm parm) {
   ...
   * @code xStreamBuffer uartRxStream;
   *
   ...
   * @code xStreamBuffer protocolStream;
   *
   ...
   * @code xStreamBuffer sensorStream;
   *
   ...
   * @endcode
   *
   * @param[out] stream_ Pointer to xStreamBuffer handle to be initialized.
   ...
   *
   * @note Streams operate on individual bytes (xByte/Byte_t). To send
   ...
   * @code xStreamBuffer channelStream = null;
   *
   ...
   * @code xStreamBuffer uartTxStream;
   *
   * void setupUART(void) {
   ...
   *
   * Example Usage:
   * @code void blinkLED(xTask task, xTaskParm parm) {
   ...
   * @endcode
   *
   * @param[out] task_          Pointer to xTask variable that will receive the
   ...
   *                            executed. Function signature: void func(xTask
   *                            task, xTaskParm parm).
   ...
   *
   * Example Usage:
   * @code void myTaskFunction(xTask task, xTaskParm parm) {
   ...
   * @endcode
   *
   * @param[out] timer_  Pointer to xTimer variable that will receive the timer
   ...
   * @code xTimer timers[MAX_SENSORS];
   * xBase timerCount = 0;
   *
   ...
   * @code xTimer systemTimers[MAX_TIMERS];
   * xBase timerCount = 0;
   *
   ...
   * @code xTimer fastTimer, mediumTimer, slowTimer;
   *
   ...
   * @code xTimer timer1, timer2, timer3;
   *
   ...
   * @code xTimer backgroundTimers[MAX_TIMERS];
   * xBase timerCount = 0;
   *
   ...
   * @code xReturn initializeFilesystem(xHalfWord blockDeviceUID, xVolume
   * volume) {
   ...
   * @code void displayVolumeDetails(xVolume vol) {
   *   xVolumeInfo info;
   *
   *   if (OK(xFSGetVolumeInfo(vol, &info))) {
   ...
   * @param[out] info_   Pointer to xVolumeInfo structure to receive volume
   ...
   * @endcode
   *
   * @param[out] file_   Pointer to xFile handle to be initialized. On success,
   ...
   * @code xReturn writeData(xVolume vol, const char *path, xByte *data, xSize
   * size) {
   *   xFile file;
   ...
   * @code void processFiles(xVolume vol) {
   *   xFile input, output;
   *
   ...
   * @code void logEvent(xVolume vol, const char *event) {
   *   xFile logFile;
   *
   ...
   * @code xWord getFileSize(xFile file) {
   *   xWord size;
   *   xWord currentPos;
   *
   ...
   * @code void copyFileWithProgress(xFile src, xFile dst) {
   *   xWord totalSize;
   ...
   * @code xBase isFileEmpty(xFile file) {
   *   xWord size;
   *
   ...
   * @code xReturn saveConfig(xVolume vol, xByte *configData, xSize dataSize) {
   *   xFile file;
   *
   ...
   * @code xReturn atomicUpdate(xFile file, xByte *newData, xSize dataSize) {
   *   xWord originalPos;
   *   xWord originalSize;
   *
   ...
   * @code xReturn readAndValidate(xFile file, xWord expectedSize) {
   *   xByte *data;
   *   xBase eof;
   *
   ...
   * @code xReturn copyFile(xFile source, xFile dest) {
   *   xBase eof = 0;
   *   xByte *buffer;
   *
   *   while (!eof) {
   ...
   * @code xBase atEndOfFile(xFile file) {
   *   xBase eof;
   *
   ...
   * @code xReturn loadConfig(xVolume vol, Config_t *config) {
   *   xBase exists;
   ...
   * @code xReturn ensureLogFile(xVolume vol) {
   *   xBase exists;
   *
   ...
   * @code xReturn validateRequiredFiles(xVolume vol) {
   ...
   * @code xReturn updateConfigFile(xVolume vol, xByte *newConfig, xSize size) {
   *   xBase exists;
   *
   ...
   * @code void cleanOldLogs(xVolume vol) {
   *   const char *oldLogs[] = {
   *     "/log.old.3",
   *     "/log.old.2",
   *     "/log.old.1"
   ...
   * @code xReturn validateAndClean(xVolume vol, const char *path) {
   *   xFile file;
   *   xBase isValid = 0;
   *
   ...
   * @code void rotateLogs(xVolume vol) {
   *   xBase exists;
   *
   ...
   * @code xReturn atomicConfigUpdate(xVolume vol, xByte *newConfig, xSize size)
   * {
   *   xFile tmpFile;
   *
   ...
   * @code xReturn archiveOldData(xVolume vol, const char *filename) {
   *   xByte oldPath[256];
   *   xByte newPath[256];
   *   xBase exists;
   *
   ...
   * @code xReturn processPath(xVolume vol, const char *path) {
   *   xDirEntry entry;
   *
   ...
   * @code void displayFileInfo(xVolume vol, const char *path) {
   *   xDirEntry entry;
   *
   *   if (OK(xFileGetInfo(vol, (xByte*)path, &entry))) {
   *     printf("Name: %s\n", entry.name);
   ...
   * @code xReturn findLargeFiles(xVolume vol, const char *dirPath, xWord
   * minSize) {
   *   xDir dir;
   *   xDirEntry entry;
   *
   ...
   *                     separators.
   * @param[out] entry_  Pointer to xDirEntry structure to receive file
   ...
   * @code xBase findFileInDirectory(xVolume vol, const char *dirPath, const
   * char *filename) {
   *   xDir dir;
   *   xDirEntry entry;
   *   xBase found = 0;
   ...
   * @code xWord calculateDirectorySize(xVolume vol, const char *path) {
   *   xDir dir;
   *   xDirEntry entry;
   *   xWord totalSize = 0;
   *
   ...
   * @code void countFileTypes(xVolume vol) {
   *   xDir dir;
   *   xDirEntry entry;
   ...
   * @endcode
   *
   * @param[out] dir_    Pointer to xDir handle to be initialized. On success,
   ...
   *
   * Example 2: Error handling with guaranteed close
   * @code xReturn processDirectory(xVolume vol, const char *path) {
   *   xDir dir;
   *   xDirEntry entry;
   *   xReturn result = ReturnError;
   *
   ...
   * @code xBase hasExtension(const char *filename, const char *ext) {
   ...
   * @param[out] entry_ Pointer to xDirEntry structure to receive entry
   *                    information.
   *
   ...
   * @code if (OK(xDirMake(vol, (xByte*)"/logs"))) {
   *   xFile file;
